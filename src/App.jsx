...
// ─── SHARED FOOTER ───────────────────────────────────────────────────────────
function SkilloraFooter() {
  return (
    <footer style={{ borderTop:'1px solid #003050', background:'#001a28', padding:'10px 40px', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'space-between', gap:'16px', flexWrap:'wrap' }}>
      <span style={{ fontSize:'0.7rem', color:'rgba(250,255,216,0.4)', lineHeight:1.5 }}>
        These are the notes I made during my JEE preparation. Every formula, every concept — written by hand, page by page. The content credit goes entirely to my incredible JEE faculties, whose teaching made all of this possible. 🙏
      </span>
      <span style={{ fontSize:'0.7rem', color:'rgba(250,255,216,0.18)', whiteSpace:'nowrap', flexShrink:0 }}>© 2025 Skillora</span>
    </footer>
  );
}
...
function LandingPage({ onGetStarted, onTryCalculator }) {
  ...
  const features = [
    ...
    { icon: '📐', title: 'Mathematics Notes', desc: '16 Class XI + 13 Class XII handwritten chapters covering the full JEE syllabus.' },
    { icon: '⚡', title: 'Physics Notes', desc: '22 Class XI + 14 Class XII topics from Mechanics to Modern Physics.' },
    { icon: '🧪', title: 'Chemistry Notes', desc: '14 Class XI + 17 Class XII chapters — Organic, Inorganic & Physical Chemistry.' },
  ];
  ...
  const stats = [
    { value: '58+', label: 'Calculators' },
    { value: '96', label: 'PDF Chapters' },
...
const matMul = (A, B) => { const r = A.length, c = B[0].length, m = B.length; return Array.from({length:r}, (_,i) => Array.from({length:c}, (_,j) => A[i].reduce((s,_,k) => s + A[i][k]*B[k][j], 0))); };
...
function SimpleAlgebra() {
  ...
  const calc = () => { const x=parseFloat(a), y=parseFloat(b); if(isNaN(x)||isNaN(y)) return setRes('Invalid input'); const r = op==='+'?x+y:op==='-'?x-y:op==='×'?x*y:op==='÷'?y!==0?x/y:'Cannot divide by zero':0; setRes(typeof r==='string'?r:`${x} ${op} ${y} = ${r}`); };
  return <div><Row><Field label="First Number"><Input value={a} onChange={e=>setA(e.target.value)} placeholder="e.g. 24"/></Field><Field label="Second Number"><Input value={b} onChange={e=>setB(e.target.value)} placeholder="e.g. 6"/></Field></Row><div style={{marginTop:'12px'}}><Label>Operation</Label><div style={{display:'flex',gap:'8px'}}>{['+','-','×','÷'].map(o=><Btn key={o} onClick={()=>setOp(o)} variant={op===o?'primary':'secondary'}>{o}</Btn>)}</div></div><Btn style={{marginTop:'16px'}} onClick={calc}>Calculate</Btn><Result>{res}</Result></div>;
}
function NumberWords() {
  const [val,setVal]=useState(''); const [mode,setMode]=useState('num2word'); const [res,setRes]=useState(null);
  const calc = () => { if(mode==='num2word') { const n=parseInt(val); setRes(isNaN(n)?'Invalid':numberToWords(n)); } else { const n=wordsToNumber(val); setRes(isNaN(n)||n===0&&val.toLowerCase()!=='zero'?'Could not parse':''+n); } };
  return <div><div style={{display:'flex',gap:'8px',marginBottom:'12px'}}><Btn onClick={()=>setMode('num2word')} variant={mode==='num2word'?'primary':'secondary'}>Number → Words</Btn><Btn onClick={()=>setMode('word2num')} variant={mode==='word2num'?'primary':'secondary'}>Words → Number</Btn></div><Field label={mode==='num2word'?'Enter Number':'Enter Words'}><Input value={val} onChange={e=>setVal(e.target.value)} placeholder={mode==='num2word'?'e.g. 5692':'e.g. five thousand six hundred ninety-two'}/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Convert</Btn><Result>{res}</Result></div>;
}
function RandomNumber() {
  ...
  const gen = () => { const lo=parseInt(min),hi=parseInt(max),c=parseInt(count); if(isNaN(lo)||isNaN(hi)||lo>hi) return setRes('Invalid range'); const nums = Array.from({length:Math.min(c,50)},()=>Math.floor(Math.random()*(hi-lo+1))+lo); setRes(nums.join(', ')); };
  return <div><Row><Field label="Min"><Input value={min} onChange={e=>setMin(e.target.value)}/></Field><Field label="Max"><Input value={max} onChange={e=>setMax(e.target.value)}/></Field></Row><div style={{marginTop:'12px'}}><Field label="Count (max 50)"><Input value={count} onChange={e=>setCount(e.target.value)} type="number" min="1" max="50"/></Field></div><Btn style={{marginTop:'12px'}} onClick={gen}>Generate</Btn><Result>{res}</Result></div>;
}
function ArrangeNumbers() {
  const [nums,setNums]=useState(''); const [order,setOrder]=useState('asc'); const [res,setRes]=useState(null);
  const calc = () => { const arr = nums.split(/[\s,]+/).map(Number).filter(x=>!isNaN(x)); if(!arr.length) return; const sorted = [...arr].sort((a,b)=>order==='asc'?a-b:b-a); setRes(sorted.join(' , ')); };
  return <div><Field label="Numbers (comma or space separated)"><Input value={nums} onChange={e=>setNums(e.target.value)} placeholder="e.g. 5, 2, 8, 1, 9"/></Field><div style={{display:'flex',gap:'8px',marginTop:'12px'}}><Btn onClick={()=>setOrder('asc')} variant={order==='asc'?'primary':'secondary'}>Ascending ↑</Btn><Btn onClick={()=>setOrder('desc')} variant={order==='desc'?'primary':'secondary'}>Descending ↓</Btn></div><Btn style={{marginTop:'12px'}} onClick={calc}>Arrange</Btn><Result>{res}</Result></div>;
}
function NumberExpansion() {
  const [num,setNum]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const n = Math.abs(parseInt(num)); if(isNaN(n)) return; const s = String(n); const parts = []; const places = [1,10,100,1000,10000,100000,1000000,10000000,100000000]; for(let i=0;i<s.length;i++){const d=parseInt(s[i]); const p=places[s.length-1-i]; if(d!==0) parts.push(`${d}×${p}`);} setRes(parts.join(' + ')); };
  return <div><Field label="Number"><Input value={num} onChange={e=>setNum(e.target.value)} placeholder="e.g. 5692"/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Expand</Btn><Result label={`Expansion of ${num}`}>{res}</Result></div>;
}
function CompareNumbers() {
  const [a,setA]=useState(''); const [b,setB]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const x=parseFloat(a),y=parseFloat(b); if(isNaN(x)||isNaN(y)) return; setRes(x>y?`${a} > ${b}`:x<y?`${a} < ${b}`:`${a} = ${b}`); };
  return <div><Row><Field label="Number A"><Input value={a} onChange={e=>setA(e.target.value)}/></Field><Field label="Number B"><Input value={b} onChange={e=>setB(e.target.value)}/></Field></Row><Btn style={{marginTop:'12px'}} onClick={calc}>Compare</Btn><Result>{res}</Result></div>;
}
function MaxMin() {
  const [nums,setNums]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const arr = nums.split(/[\s,]+/).map(Number).filter(x=>!isNaN(x)); if(!arr.length) return; setRes(`Maximum: ${Math.max(...arr)}   |   Minimum: ${Math.min(...arr)}`); };
  return <div><Field label="Numbers (comma or space separated)"><Input value={nums} onChange={e=>setNums(e.target.value)} placeholder="e.g. 4, 7, 2, 9, 1"/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Find Max &amp; Min</Btn><Result>{res}</Result></div>;
}
...
function SimplifyFraction() {
  ...
  const calc = () => { const num=parseInt(n),den=parseInt(d); if(isNaN(num)||isNaN(den)||den===0) return setRes('Invalid'); const [sn,sd]=simplifyFraction(num,den); setRes(sd===1?`${sn}`:sn===num&&sd===den?`${num}/${den} is already in simplest form`:`${num}/${den} = ${sn}/${sd}`); };
  return <div><Row><Field label="Numerator"><Input value={n} onChange={e=>setN(e.target.value)}/></Field><Field label="Denominator"><Input value={d} onChange={e=>setD(e.target.value)}/></Field></Row><Btn style={{marginTop:'12px'}} onClick={calc}>Simplify</Btn><Result>{res}</Result></div>;
}
function ClockAngle() {
  const [h,setH]=useState(''); const [m,setM]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const hr=parseFloat(h)%12,mn=parseFloat(m); if(isNaN(hr)||isNaN(mn)||mn<0||mn>=60) return setRes('Invalid'); const hourAngle = hr*30 + mn*0.5; const minAngle = mn*6; const diff = Math.abs(hourAngle-minAngle); const angle = Math.min(diff, 360-diff); setRes(`Angle between hands = ${angle.toFixed(2)}°`); };
  return <div><Row><Field label="Hour (0–12)"><Input value={h} onChange={e=>setH(e.target.value)} placeholder="e.g. 3"/></Field><Field label="Minutes (0–59)"><Input value={m} onChange={e=>setM(e.target.value)} placeholder="e.g. 30"/></Field></Row><Btn style={{marginTop:'12px'}} onClick={calc}>Calculate Angle</Btn><Result>{res}</Result></div>;
}
...
function AngleDecider() {
  ...
  const calc = () => { const v=parseFloat(a); if(isNaN(v)||v<0||v>360) return setRes('Enter angle between 0 and 360'); const t = v===0?'Zero':v<90?'Acute':v===90?'Right':v<180?'Obtuse':v===180?'Straight':v<360?'Reflex':'Full Angle'; setRes(`${v}° is a ${t} angle`); };
  return <div><Field label="Angle (degrees)"><Input value={a} onChange={e=>setA(e.target.value)} placeholder="e.g. 45"/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Classify</Btn><Result>{res}</Result></div>;
}
function MixedImproper() {
  const [mode,setMode]=useState('mixed2imp'); const [w,setW]=useState(''); const [n,setN]=useState(''); const [d,setD]=useState(''); const [imp,setImp]=useState(''); const [impD,setImpD]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { if(mode==='mixed2imp') { const W=parseInt(w),N=parseInt(n),D=parseInt(d); if(isNaN(W)||isNaN(N)||isNaN(D)||D===0) return setRes('Invalid'); setRes(`${W} ${N}/${D} = ${W*D+N}/${D}`); } else { const num=parseInt(imp),den=parseInt(impD); if(isNaN(num)||isNaN(den)||den===0) return setRes('Invalid'); const whole=Math.floor(num/den),rem=num%den; setRes(rem===0?`${whole}`:whole===0?`${rem}/${den}`:`${whole} ${rem}/${den}`); } };
  return <div><div style={{display:'flex',gap:'8px',marginBottom:'12px'}}><Btn onClick={()=>setMode('mixed2imp')} variant={mode==='mixed2imp'?'primary':'secondary'}>Mixed → Improper</Btn><Btn onClick={()=>setMode('imp2mixed')} variant={mode==='imp2mixed'?'primary':'secondary'}>Improper → Mixed</Btn></div>{mode==='mixed2imp'?<><Row><Field label="Whole"><Input value={w} onChange={e=>setW(e.target.value)}/></Field><Field label="Numerator"><Input value={n} onChange={e=>setN(e.target.value)}/></Field></Row><div style={{marginTop:'12px'}}><Field label="Denominator"><Input value={d} onChange={e=>setD(e.target.value)}/></Field></div></>:<Row><Field label="Numerator"><Input value={imp} onChange={e=>setImp(e.target.value)}/></Field><Field label="Denominator"><Input value={impD} onChange={e=>setImpD(e.target.value)}/></Field></Row>}<Btn style={{marginTop:'12px'}} onClick={calc}>Convert</Btn><Result>{res}</Result></div>;
}
function FractionDecimal() {
  const [mode,setMode]=useState('frac2dec'); const [n,setN]=useState(''); const [d,setD]=useState(''); const [dec,setDec]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { if(mode==='frac2dec') { const num=parseInt(n),den=parseInt(d); if(isNaN(num)||isNaN(den)||den===0) return setRes('Invalid'); setRes(`${num}/${den} = ${(num/den).toString()}`); } else { const v=parseFloat(dec); if(isNaN(v)) return setRes('Invalid'); const str=v.toString(); const decIdx=str.indexOf('.'); if(decIdx===-1) return setRes(`${v}/1`); const decPlaces=str.length-decIdx-1; const denom=10**decPlaces; const numer=Math.round(v*denom); const [sn,sd]=simplifyFraction(numer,denom); setRes(`${dec} = ${sn}/${sd}`); } };
  return <div><div style={{display:'flex',gap:'8px',marginBottom:'12px'}}><Btn onClick={()=>setMode('frac2dec')} variant={mode==='frac2dec'?'primary':'secondary'}>Fraction → Decimal</Btn><Btn onClick={()=>setMode('dec2frac')} variant={mode==='dec2frac'?'primary':'secondary'}>Decimal → Fraction</Btn></div>{mode==='frac2dec'?<Row><Field label="Numerator"><Input value={n} onChange={e=>setN(e.target.value)}/></Field><Field label="Denominator"><Input value={d} onChange={e=>setD(e.target.value)}/></Field></Row>:<Field label="Decimal"><Input value={dec} onChange={e=>setDec(e.target.value)} placeholder="e.g. 0.75"/></Field>}<Btn style={{marginTop:'12px'}} onClick={calc}>Convert</Btn><Result>{res}</Result></div>;
}
function DecimalOps() {
  const [a,setA]=useState(''); const [b,setB]=useState(''); const [op,setOp]=useState('+'); const [res,setRes]=useState(null);
  const calc = () => { const x=parseFloat(a),y=parseFloat(b); if(isNaN(x)||isNaN(y)) return; const r = op==='+'?x+y:op==='-'?x-y:op==='×'?x*y:y!==0?x/y:'Cannot divide by zero'; setRes(typeof r==='string'?r:`${a} ${op} ${b} = ${parseFloat(r.toFixed(10))}`); };
  return <div><Row><Field label="First Decimal"><Input value={a} onChange={e=>setA(e.target.value)} placeholder="e.g. 3.14"/></Field><Field label="Second Decimal"><Input value={b} onChange={e=>setB(e.target.value)} placeholder="e.g. 2.71"/></Field></Row><div style={{marginTop:'12px'}}><Label>Operation</Label><div style={{display:'flex',gap:'8px'}}>{['+','-','×','÷'].map(o=><Btn key={o} onClick={()=>setOp(o)} variant={op===o?'primary':'secondary'}>{o}</Btn>)}</div></div><Btn style={{marginTop:'16px'}} onClick={calc}>Calculate</Btn><Result>{res}</Result></div>;
}
function PrimeCheck() {
  const [n,setN]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const v=parseInt(n); if(isNaN(v)) return; setRes(`${v} is ${isPrime(v)?'':'NOT '}a prime number.`+(isPrime(v)?'':v<2?'':` It is divisible by ${primeFactors(v)[0]}.`)); };
  return <div><Field label="Number"><Input value={n} onChange={e=>setN(e.target.value)} placeholder="e.g. 97"/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Check</Btn><Result>{res}</Result></div>;
}
function IntegerOps() {
  const [a,setA]=useState(''); const [b,setB]=useState(''); const [op,setOp]=useState('+'); const [res,setRes]=useState(null);
  const calc = () => { const x=parseInt(a),y=parseInt(b); if(isNaN(x)||isNaN(y)) return; const r=op==='+'?x+y:op==='-'?x-y:op==='×'?x*y:op==='÷'?y!==0?Math.trunc(x/y):'Cannot divide by zero':op==='mod'?y!==0?x%y:'Cannot mod by zero':0; setRes(typeof r==='string'?r:`${x} ${op} ${y} = ${r}`); };
  return <div><Row><Field label="Integer A"><Input value={a} onChange={e=>setA(e.target.value)} placeholder="e.g. -5"/></Field><Field label="Integer B"><Input value={b} onChange={e=>setB(e.target.value)} placeholder="e.g. 3"/></Field></Row><div style={{marginTop:'12px'}}><Label>Operation</Label><div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>{['+','-','×','÷','mod'].map(o=><Btn key={o} onClick={()=>setOp(o)} variant={op===o?'primary':'secondary'}>{o}</Btn>)}</div></div><Btn style={{marginTop:'16px'}} onClick={calc}>Calculate</Btn><Result>{res}</Result></div>;
}
function CompareIntegers() {
  const [a,setA]=useState(''); const [b,setB]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const x=parseInt(a),y=parseInt(b); if(isNaN(x)||isNaN(y)) return; setRes(x>y?`${x} > ${y}`:x<y?`${x} < ${y}`:`${x} = ${y}`); };
  return <div><Row><Field label="Integer A"><Input value={a} onChange={e=>setA(e.target.value)} placeholder="e.g. -7"/></Field><Field label="Integer B"><Input value={b} onChange={e=>setB(e.target.value)} placeholder="e.g. -3"/></Field></Row><Btn style={{marginTop:'12px'}} onClick={calc}>Compare</Btn><Result>{res}</Result></div>;
}
function FractionOps() {
  ...
  const calc = () => { const a=parseInt(n1),b=parseInt(d1),c=parseInt(n2),dd=parseInt(d2); if([a,b,c,dd].some(isNaN)||b===0||dd===0) return setRes('Invalid'); let rn,rd; if(op==='+'){rn=a*dd+c*b;rd=b*dd;}else if(op==='-'){rn=a*dd-c*b;rd=b*dd;}else if(op==='×'){rn=a*c;rd=b*dd;}else{rn=a*dd;rd=b*c;} const [sn,sd]=simplifyFraction(rn,rd); setRes(`${a}/${b} ${op} ${c}/${dd} = ${sn}/${sd}`+(rn!==sn?` (simplified from ${rn}/${rd})`:'')); };
  return <div><Row><Field label="Numerator 1"><Input value={n1} onChange={e=>setN1(e.target.value)}/></Field><Field label="Denominator 1"><Input value={d1} onChange={e=>setD1(e.target.value)}/></Field></Row><div style={{margin:'12px 0'}}><Label>Operation</Label><div style={{display:'flex',gap:'8px'}}>{['+','-','×','÷'].map(o=><Btn key={o} onClick={()=>setOp(o)} variant={op===o?'primary':'secondary'}>{o}</Btn>)}</div></div><Row><Field label="Numerator 2"><Input value={n2} onChange={e=>setN2(e.target.value)}/></Field><Field label="Denominator 2"><Input value={d2} onChange={e=>setD2(e.target.value)}/></Field></Row><Btn style={{marginTop:'12px'}} onClick={calc}>Calculate</Btn><Result>{res}</Result></div>;
}
function CompareFractions() {
  ...
  const calc = () => { const a=parseInt(n1),b=parseInt(d1),c=parseInt(n2),dd=parseInt(d2); if([a,b,c,dd].some(isNaN)||b===0||dd===0) return; const l=a*dd,r=c*b; setRes(l>r?`${a}/${b} > ${c}/${dd}`:l<r?`${a}/${b} < ${c}/${dd}`:`${a}/${b} = ${c}/${dd}`); };
  return <div><Row><Field label="Numerator 1"><Input value={n1} onChange={e=>setN1(e.target.value)}/></Field><Field label="Denominator 1"><Input value={d1} onChange={e=>setD1(e.target.value)}/></Field></Row><div style={{height:'12px'}}/><Row><Field label="Numerator 2"><Input value={n2} onChange={e=>setN2(e.target.value)}/></Field><Field label="Denominator 2"><Input value={d2} onChange={e=>setD2(e.target.value)}/></Field></Row><Btn style={{marginTop:'12px'}} onClick={calc}>Compare</Btn><Result>{res}</Result></div>;
}
function Statistics() {
  const [nums,setNums]=useState(''); const [res,setRes]=useState(null);
...
function PythagoreanTriplet() {
  ...
  const calc = () => { const x=parseInt(a),y=parseInt(b),z=parseInt(c); if([x,y,z].some(isNaN)||[x,y,z].some(v=>v<=0)) return; const sides=[x,y,z].sort((a,b)=>a-b); const is=sides[0]**2+sides[1]**2===sides[2]**2; setRes(`(${sides.join(', ')}) is ${is?'':'NOT '}a Pythagorean triplet.${is?`\n${sides[0]}² + ${sides[1]}² = ${sides[2]}²`:''}`); };
  return <div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'12px'}}><Field label="Side a"><Input value={a} onChange={e=>setA(e.target.value)}/></Field><Field label="Side b"><Input value={b} onChange={e=>setB(e.target.value)}/></Field><Field label="Side c"><Input value={c} onChange={e=>setC(e.target.value)}/></Field></div><Btn style={{marginTop:'12px'}} onClick={calc}>Check</Btn><Result>{res}</Result></div>;
}
function DecimalPercent() {
  const [mode,setMode]=useState('d2p'); const [val,setVal]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const v=parseFloat(val); if(isNaN(v)) return; setRes(mode==='d2p'?`${v} = ${v*100}%`:`${v}% = ${v/100}`); };
  return <div><div style={{display:'flex',gap:'8px',marginBottom:'12px'}}><Btn onClick={()=>setMode('d2p')} variant={mode==='d2p'?'primary':'secondary'}>Decimal → %</Btn><Btn onClick={()=>setMode('p2d')} variant={mode==='p2d'?'primary':'secondary'}>% → Decimal</Btn></div><Field label={mode==='d2p'?'Decimal':'Percentage'}><Input value={val} onChange={e=>setVal(e.target.value)} placeholder={mode==='d2p'?'e.g. 0.75':'e.g. 75'}/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Convert</Btn><Result>{res}</Result></div>;
}
function RatioPercent() {
  ...
  const calc = () => { if(mode==='r2p') { const x=parseFloat(a),y=parseFloat(b); if(isNaN(x)||isNaN(y)||y===0) return; setRes(`${x}:${y} = ${(x/(x+y)*100).toFixed(4)}% : ${(y/(x+y)*100).toFixed(4)}%`); } else { const v=parseFloat(p); if(isNaN(v)) return; const g=gcd(Math.round(v*100),10000); setRes(`${v}% = ${Math.round(v*100)/g} : ${10000/g}`); } };
  return <div><div style={{display:'flex',gap:'8px',marginBottom:'12px'}}><Btn onClick={()=>setMode('r2p')} variant={mode==='r2p'?'primary':'secondary'}>Ratio → %</Btn><Btn onClick={()=>setMode('p2r')} variant={mode==='p2r'?'primary':'secondary'}>% → Ratio</Btn></div>{mode==='r2p'?<Row><Field label="a"><Input value={a} onChange={e=>setA(e.target.value)}/></Field><Field label="b"><Input value={b} onChange={e=>setB(e.target.value)}/></Field></Row>:<Field label="Percentage"><Input value={p} onChange={e=>setP(e.target.value)} placeholder="e.g. 25"/></Field>}<Btn style={{marginTop:'12px'}} onClick={calc}>Convert</Btn><Result>{res}</Result></div>;
}
function Factors() {
  const [n,setN]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const v=parseInt(n); if(isNaN(v)||v<1) return; const f=[]; for(let i=1;i<=v;i++) if(v%i===0) f.push(i); setRes(`Factors of ${v}: ${f.join(', ')}\nTotal: ${f.length} factors`); };
  return <div><Field label="Number"><Input value={n} onChange={e=>setN(e.target.value)} placeholder="e.g. 48"/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Find Factors</Btn><Result>{res}</Result></div>;
}
function ProfitLoss() {
...
function RationalOps() {
  const [n1,setN1]=useState(''); const [d1,setD1]=useState(''); const [n2,setN2]=useState(''); const [d2,setD2]=useState(''); const [op,setOp]=useState('+'); const [res,setRes]=useState(null);
  const calc = () => { const a=parseInt(n1),b=parseInt(d1),c=parseInt(n2),dd=parseInt(d2); if([a,b,c,dd].some(isNaN)||b===0||dd===0) return setRes('Invalid'); let rn,rd; if(op==='+'){rn=a*dd+c*b;rd=b*dd;}else if(op==='-'){rn=a*dd-c*b;rd=b*dd;}else if(op==='×'){rn=a*c;rd=b*dd;}else{if(c===0)return setRes('Cannot divide by zero');rn=a*dd;rd=b*c;} const [sn,sd]=simplifyFraction(rn,rd); setRes(`= ${sn}/${sd}${sn!==rn?' (simplified)':''}`); };
  return <div><Row><Field label="Numerator 1"><Input value={n1} onChange={e=>setN1(e.target.value)}/></Field><Field label="Denominator 1"><Input value={d1} onChange={e=>setD1(e.target.value)}/></Field></Row><div style={{margin:'12px 0'}}><Label>Operation</Label><div style={{display:'flex',gap:'8px'}}>{['+','-','×','÷'].map(o=><Btn key={o} onClick={()=>setOp(o)} variant={op===o?'primary':'secondary'}>{o}</Btn>)}</div></div><Row><Field label="Numerator 2"><Input value={n2} onChange={e=>setN2(e.target.value)}/></Field><Field label="Denominator 2"><Input value={d2} onChange={e=>setD2(e.target.value)}/></Field></Row><Btn style={{marginTop:'12px'}} onClick={calc}>Calculate</Btn><Result>{res}</Result></div>;
}
function ReduceRational() {
  const [n,setN]=useState(''); const [d,setD]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const num=parseInt(n),den=parseInt(d); if(isNaN(num)||isNaN(den)||den===0) return; const [sn,sd]=simplifyFraction(num,den); const g=gcd(Math.abs(num),Math.abs(den)); setRes(`${num}/${den} = ${sn}/${sd}\nGCD used: ${g}`); };
  return <div><Row><Field label="Numerator"><Input value={n} onChange={e=>setN(e.target.value)}/></Field><Field label="Denominator"><Input value={d} onChange={e=>setD(e.target.value)}/></Field></Row><Btn style={{marginTop:'12px'}} onClick={calc}>Reduce</Btn><Result>{res}</Result></div>;
}
function CompareRational() {
  const [n1,setN1]=useState(''); const [d1,setD1]=useState(''); const [n2,setN2]=useState(''); const [d2,setD2]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const a=parseInt(n1),b=parseInt(d1),c=parseInt(n2),dd=parseInt(d2); if([a,b,c,dd].some(isNaN)||b===0||dd===0) return; const l=a*dd,r2=c*b; setRes(l>r2?`${a}/${b} > ${c}/${dd}`:l<r2?`${a}/${b} < ${c}/${dd}`:`${a}/${b} = ${c}/${dd}`); };
  return <div><Row><Field label="Numerator 1"><Input value={n1} onChange={e=>setN1(e.target.value)}/></Field><Field label="Denominator 1"><Input value={d1} onChange={e=>setD1(e.target.value)}/></Field></Row><div style={{height:'12px'}}/><Row><Field label="Numerator 2"><Input value={n2} onChange={e=>setN2(e.target.value)}/></Field><Field label="Denominator 2"><Input value={d2} onChange={e=>setD2(e.target.value)}/></Field></Row><Btn style={{marginTop:'12px'}} onClick={calc}>Compare</Btn><Result>{res}</Result></div>;
}
function PrimeFactorization() {
  const [n,setN]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const v=parseInt(n); if(isNaN(v)||v<2) return setRes('Enter an integer ≥ 2'); const f=primeFactors(v); const grouped={};f.forEach(p=>grouped[p]=(grouped[p]||0)+1); const expr=Object.entries(grouped).map(([p,e])=>e>1?`${p}^${e}`:p).join(' × '); setRes(`${v} = ${expr}`); };
  return <div><Field label="Number"><Input value={n} onChange={e=>setN(e.target.value)} placeholder="e.g. 360"/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Factorize</Btn><Result>{res}</Result></div>;
}
...
function Factorial2() {
  const [n,setN]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const v=parseInt(n); if(isNaN(v)||v<0) return setRes('Enter a non-negative integer'); if(v>20) return setRes('Too large (max 20 for exact)'); setRes(`${v}! = ${factorial(v)}`); };
  return <div><Field label="n (non-negative integer, max 20)"><Input value={n} onChange={e=>setN(e.target.value)} placeholder="e.g. 7"/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Calculate</Btn><Result>{res}</Result></div>;
}
function Discount() {
...
function DistanceFormula() {
  ...
  const calc = () => { const a=parseFloat(x1),b=parseFloat(y1),c=parseFloat(x2),d=parseFloat(y2); if([a,b,c,d].some(isNaN)) return; const dist=Math.sqrt((c-a)**2+(d-b)**2); setRes(`Distance = √((${c}-${a})² + (${d}-${b})²) = √${(c-a)**2+(d-b)**2} = ${parseFloat(dist.toFixed(8))}`); };
  return <div><Row><Field label="x₁"><Input value={x1} onChange={e=>setX1(e.target.value)}/></Field><Field label="y₁"><Input value={y1} onChange={e=>setY1(e.target.value)}/></Field></Row><div style={{height:'12px'}}/><Row><Field label="x₂"><Input value={x2} onChange={e=>setX2(e.target.value)}/></Field><Field label="y₂"><Input value={y2} onChange={e=>setY2(e.target.value)}/></Field></Row><Btn style={{marginTop:'12px'}} onClick={calc}>Calculate</Btn><Result>{res}</Result></div>;
}
function SectionFormula() {
  const [x1,setX1]=useState(''); const [y1,setY1]=useState(''); const [x2,setX2]=useState(''); const [y2,setY2]=useState(''); const [m,setM]=useState(''); const [n,setN]=useState(''); const [type,setType]=useState('internal'); const [res,setRes]=useState(null);
  const calc = () => { const a=parseFloat(x1),b=parseFloat(y1),c=parseFloat(x2),d=parseFloat(y2),M=parseFloat(m),N=parseFloat(n); if([a,b,c,d,M,N].some(isNaN)) return; const px = type==='internal'?(M*c+N*a)/(M+N):(M*c-N*a)/(M-N); const py = type==='internal'?(M*d+N*b)/(M+N):(M*d-N*b)/(M-N); setRes(`Point = (${parseFloat(px.toFixed(6))}, ${parseFloat(py.toFixed(6))})`); };
  return <div><Row><Field label="x₁"><Input value={x1} onChange={e=>setX1(e.target.value)}/></Field><Field label="y₁"><Input value={y1} onChange={e=>setY1(e.target.value)}/></Field></Row><div style={{height:'12px'}}/><Row><Field label="x₂"><Input value={x2} onChange={e=>setX2(e.target.value)}/></Field><Field label="y₂"><Input value={y2} onChange={e=>setY2(e.target.value)}/></Field></Row><div style={{height:'12px'}}/><Row><Field label="m (ratio part 1)"><Input value={m} onChange={e=>setM(e.target.value)}/></Field><Field label="n (ratio part 2)"><Input value={n} onChange={e=>setN(e.target.value)}/></Field></Row><div style={{display:'flex',gap:'8px',marginTop:'12px'}}><Btn onClick={()=>setType('internal')} variant={type==='internal'?'primary':'secondary'}>Internal</Btn><Btn onClick={()=>setType('external')} variant={type==='external'?'primary':'secondary'}>External</Btn></div><Btn style={{marginTop:'12px'}} onClick={calc}>Calculate</Btn><Result>{res}</Result></div>;
}
function SlopeCalc() {
  const [x1,setX1]=useState(''); const [y1,setY1]=useState(''); const [x2,setX2]=useState(''); const [y2,setY2]=useState(''); const [res,setRes]=useState(null);
...
function DegRad() {
  ...
  const calc = () => { const v=parseFloat(val); if(isNaN(v)) return; setRes(mode==='d2r'?`${v}° = ${parseFloat((v*Math.PI/180).toFixed(8))} rad`:`${v} rad = ${parseFloat((v*180/Math.PI).toFixed(8))}°`); };
  return <div><div style={{display:'flex',gap:'8px',marginBottom:'12px'}}><Btn onClick={()=>setMode('d2r')} variant={mode==='d2r'?'primary':'secondary'}>Degrees → Radians</Btn><Btn onClick={()=>setMode('r2d')} variant={mode==='r2d'?'primary':'secondary'}>Radians → Degrees</Btn></div><Field label="Value"><Input value={val} onChange={e=>setVal(e.target.value)} placeholder={mode==='d2r'?'e.g. 90':'e.g. 1.5708'}/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Convert</Btn><Result>{res}</Result></div>;
}
function ComplexOps() {
...
function Fibonacci() {
  const [n,setN]=useState(''); const [res,setRes]=useState(null);
  const calc = () => { const v=parseInt(n); if(isNaN(v)||v<1||v>50) return setRes('Enter between 1 and 50'); const seq=[0,1]; for(let i=2;i<v;i++) seq.push(seq[i-1]+seq[i-2]); setRes(seq.slice(0,v).join(', ')); };
  return <div><Field label="Number of terms (1–50)"><Input value={n} onChange={e=>setN(e.target.value)} placeholder="e.g. 10"/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Generate</Btn><Result label="Fibonacci Sequence">{res}</Result></div>;
}
...
function MatrixOps() {
  ...
  const [size,setSize]=useState(2); const [A,setA]=useState(empty2()); const [B,setB]=useState(empty2()); const [op,setOp]=useState('add'); const [res,setRes]=useState(null);
...
function DecBin() {
  ...
  const calc=()=>{ if(mode==='d2b'){const v=parseInt(val);if(isNaN(v))return;setRes(`${v} (decimal) = ${decimalToBinary(v)} (binary)`);}else{if(!/^[01]+$/.test(val))return setRes('Invalid binary');setRes(`${val} (binary) = ${binaryToDecimal(val)} (decimal)`);} };
  return <div><div style={{display:'flex',gap:'8px',marginBottom:'12px'}}><Btn onClick={()=>setMode('d2b')} variant={mode==='d2b'?'primary':'secondary'}>Decimal → Binary</Btn><Btn onClick={()=>setMode('b2d')} variant={mode==='b2d'?'primary':'secondary'}>Binary → Decimal</Btn></div><Field label={mode==='d2b'?'Decimal Number':'Binary Number'}><Input value={val} onChange={e=>setVal(e.target.value)} placeholder={mode==='d2b'?'e.g. 42':'e.g. 101010'}/></Field><Btn style={{marginTop:'12px'}} onClick={calc}>Convert</Btn><Result>{res}</Result></div>;
}
function SurfaceAreaVolume() { return <Mensuration/>; }

// ─── TOPIC REGISTRY ─────────────────────────────────────────────────────────
const categories = [
  { name:'Basic Arithmetic', icon:'🔢', topics:[{id:1,name:'Simple Algebra',desc:'Add, subtract, multiply, divide',component:SimpleAlgebra},{id:2,name:'Number ↔ Words',desc:'Convert numbers to/from words',component:NumberWords},{id:3,name:'Random Number',desc:'Generate random numbers',component:RandomNumber},{id:4,name:'Sort Numbers',desc:'Ascending & descending order',component:ArrangeNumbers},{id:5,name:'Number Expansion',desc:'Expanded form of numbers',component:NumberExpansion},{id:6,name:'Compare Numbers',desc:'Greater, lesser, or equal',component:CompareNumbers},{id:7,name:'Max & Min',desc:'Find maximum and minimum',component:MaxMin},{id:8,name:'Multiplication Table',desc:'Generate times tables',component:MultiplicationTable}] },
  { name:'Fractions & Decimals', icon:'½', topics:[{id:9,name:'Simplify Fraction',desc:'Reduce to simplest form',component:SimplifyFraction},{id:13,name:'Mixed ↔ Improper',desc:'Convert fraction types',component:MixedImproper},{id:14,name:'Fraction ↔ Decimal',desc:'Convert between forms',component:FractionDecimal},{id:15,name:'Decimal Operations',desc:'+, -, ×, ÷ on decimals',component:DecimalOps},{id:19,name:'Fraction Operations',desc:'+, -, ×, ÷ on fractions',component:FractionOps},{id:20,name:'Compare Fractions',desc:'Which fraction is larger?',component:CompareFractions}] },
  { name:'Integers & Number Theory', icon:'ℤ', topics:[{id:16,name:'Prime Number Check',desc:'Is it prime?',component:PrimeCheck},{id:17,name:'Integer Operations',desc:'Operations on integers',component:IntegerOps},{id:18,name:'Compare Integers',desc:'Compare signed integers',component:CompareIntegers},{id:27,name:'Factors',desc:'Find all factors of a number',component:Factors},{id:32,name:'Prime Factorization',desc:'Express as prime factors',component:PrimeFactorization},{id:33,name:'Squares & Roots',desc:'Square and square root',component:SquaresRoots},{id:34,name:'Cubes & Cube Roots',desc:'Cube and cube root',component:CubesRoots},{id:35,name:'GCD & LCM',desc:'Greatest common divisor & LCM',component:GCDLCM},{id:36,name:'Factorial',desc:'Calculate n!',component:Factorial2},{id:53,name:'Fibonacci Series',desc:'Generate Fibonacci sequence',component:Fibonacci}] },
  { name:'Percentages & Finance', icon:'%', topics:[{id:25,name:'Decimal ↔ Percent',desc:'Convert between forms',component:DecimalPercent},{id:26,name:'Ratio ↔ Percent',desc:'Convert ratios and percentages',component:RatioPercent},{id:28,name:'Profit & Loss',desc:'Calculate profit/loss %',component:ProfitLoss},{id:37,name:'Discount',desc:'Marked price & discounts',component:Discount},{id:38,name:'SI & CI',desc:'Simple & compound interest',component:SICI}] },
  { name:'Rational Numbers', icon:'ℚ', topics:[{id:29,name:'Rational Operations',desc:'+, -, ×, ÷ on rationals',component:RationalOps},{id:30,name:'Reduce Rational',desc:'Simplify rational numbers',component:ReduceRational},{id:31,name:'Compare Rationals',desc:'Compare two rationals',component:CompareRational}] },
  { name:'Statistics', icon:'📊', topics:[{id:21,name:'Mean, Median, Mode',desc:'Measures of central tendency',component:Statistics}] },
  { name:'Algebra', icon:'𝑥', topics:[{id:22,name:'Linear Equation',desc:'Solve ax + b = 0',component:LinearEq},{id:39,name:'Algebraic Identities',desc:'Evaluate all standard identities',component:AlgebraicIdentities},{id:40,name:'Polynomial Division',desc:'Divide polynomials',component:PolyDivision},{id:41,name:'System of Equations',desc:'Solve 2×2 linear system',component:SystemLinear},{id:43,name:'Quadratic Equation',desc:'Solve ax² + bx + c = 0',component:QuadraticEq}] },
  { name:'Geometry', icon:'📐', topics:[{id:10,name:'Clock Angle',desc:'Angle between clock hands',component:ClockAngle},{id:11,name:'Mensuration',desc:'Area, perimeter, volume of shapes',component:Mensuration},{id:12,name:'Angle Classifier',desc:'Acute, right, obtuse, reflex',component:AngleDecider},{id:23,name:'Comp. & Supp. Angles',desc:'Complementary & supplementary',component:CompSupAngles},{id:24,name:'Pythagorean Triplet',desc:'Check if sides form a triplet',component:PythagoreanTriplet},{id:42,name:'Surface Area & Volume',desc:'3D shape calculations',component:SurfaceAreaVolume},{id:44,name:'Distance Formula',desc:'Distance between two points',component:DistanceFormula},{id:45,name:'Section Formula',desc:'Point dividing a line segment',component:SectionFormula},{id:46,name:'Slope Calculator',desc:'Slope of a line',component:SlopeCalc},{id:47,name:'Equation of Line',desc:'Find line equation from 2 points',component:LineEquation},{id:54,name:'Angle Between Lines',desc:'Angle between two lines',component:AngleBetweenLines},{id:49,name:'Triangle Operations',desc:'Area, angles, type of triangle',component:TriangleOps}] },
  { name:'Trigonometry', icon:'sin', topics:[{id:48,name:'Trig Ratios',desc:'sin, cos, tan, cosec, sec, cot',component:Trigonometry},{id:50,name:'Degrees ↔ Radians',desc:'Convert angle units',component:DegRad}] },
  { name:'Advanced Math', icon:'∑', topics:[{id:51,name:'Complex Numbers',desc:'+, -, ×, ÷ on complex numbers',component:ComplexOps},{id:52,name:'nPr & nCr',desc:'Permutations and combinations',component:Permutation},{id:55,name:'Matrix Operations',desc:'Add, subtract, multiply, transpose, inverse',component:MatrixOps},{id:56,name:'Determinants',desc:'Calculate matrix determinant',component:Determinant},{id:57,name:'Vector Operations',desc:'Add, subtract, dot/cross product',component:VectorOps},{id:58,name:'Decimal ↔ Binary',desc:'Convert number bases',component:DecBin}] },
];
...
// ─── NOTES COMPONENT ─────────────────────────────────────────────────────────
function NotesSection({ registered, onSignup }) {
  ...
  // 90-second auto-signup timer — only for unregistered users
  useEffect(() => {
    if (registered || timerFired) return;
    if (timeLeft <= 0) {
      setTimerFired(true);
      onSignup();
      return;
    ...
    return () => clearTimeout(t);
  }, [timeLeft, registered, timerFired]);

  const subject    = notesData[activeSubject];
  const classData  = subject.classes[activeClass];
  const accentColor = activeSubject === 0 ? '#ecffb0' : activeSubject === 1 ? '#226ce0' : '#34d399';

  const handleChapterClick = (ch) => {
    setActiveChapter(ch);
  };

  // ── PDF Viewer (for unregistered preview) ────────────────────────────────
  if (activeChapter) {
    return (
      <div style={{ flex:1, display:'flex', flexDirection:'column', background:'#00171f', overflowY:'auto' }}>

        {/* Header */}
        <div style={{ background:'#001e2a', borderBottom:'1px solid #003050', padding:'16px 32px', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0, flexWrap:'wrap', gap:'12px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'14px' }}>
            <button onClick={() => setActiveChapter(null)}
              style={{ background:'#003050', border:'1px solid #003a5c', borderRadius:'8px', color:'rgba(250,255,216,0.8)', padding:'7px 14px', cursor:'pointer', fontFamily:'DM Sans', fontSize:'0.82rem', display:'flex', alignItems:'center', gap:'6px' }}>
              ← Back
            </button>
            <div>
              <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'3px' }}>
                <span style={{ background:accentColor+'22', color:accentColor, borderRadius:'4px', padding:'2px 8px', fontSize:'0.65rem', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em' }}>{subject.subject}</span>
                <span style={{ color:'rgba(250,255,216,0.3)', fontSize:'0.75rem' }}>{classData.label}</span>
              </div>
              <div style={{ fontFamily:'Playfair Display', fontSize:'1.3rem', fontWeight:700, color:'#faffd8' }}>{activeChapter.name}</div>
            </div>
          </div>
          {!registered && (
            <button onClick={onSignup}
              style={{ padding:'10px 24px', borderRadius:'8px', background:'#226ce0', border:'none', cursor:'pointer', fontFamily:'DM Sans', fontWeight:700, fontSize:'0.88rem', color:'#faffd8' }}>
              🔓 Unlock Full Notes — Free
            </button>
          )}
        </div>

        {/* PDF preview with fade lock */}
        <div style={{ flex:1, position:'relative', minHeight:'600px' }}>
          ...
          />
          {/* Hide Drive top bar */}
          <div style={{ position:'absolute', top:0, left:0, right:0, height:'46px', background:'#00171f', zIndex:5 }}/>
          {/* Gradient lock overlay — unregistered only */}
          {!registered && <div style={{ position:'absolute', bottom:0, left:0, right:0, height:'52%',
            background:'linear-gradient(to bottom, transparent 0%, #00171f 58%)',
            zIndex:6, display:'flex', flexDirection:'column', justifyContent:'flex-end', alignItems:'center', paddingBottom:'40px', gap:'12px' }}>
            <div style={{ textAlign:'center', background:'#001e2a', border:'1px solid #003050', borderRadius:'16px', padding:'24px 36px', boxShadow:'0 12px 48px rgba(0,0,0,0.7)', maxWidth:'420px', width:'90%' }}>
              <div style={{ fontSize:'1.6rem', marginBottom:'8px' }}>📖</div>
              <div style={{ fontFamily:'Playfair Display', fontSize:'1.1rem', fontWeight:700, color:'#faffd8', marginBottom:'6px' }}>Reading more requires a free account</div>
              <div style={{ fontSize:'0.8rem', color:'rgba(250,255,216,0.45)', marginBottom:'18px', lineHeight:1.6 }}>Sign up in 30 seconds — completely free, no payment needed</div>
              <button onClick={onSignup}
                style={{ padding:'12px 32px', borderRadius:'10px', background:'#226ce0', border:'none', cursor:'pointer', fontFamily:'DM Sans', fontWeight:700, fontSize:'0.92rem', color:'#faffd8', width:'100%' }}>
                Create Free Account →
              </button>
            </div>
          </div>}
          <div style={{ position:'absolute', bottom:'12px', right:'20px', zIndex:7, fontSize:'0.65rem', color:'rgba(255,255,255,0.06)', pointerEvents:'none' }}>© Skillora</div>
        </div>
      </div>
    );
  }

  // ── Chapter List (main view) ──────────────────────────────────────────────
  return (
    <div style={{ flex:1, overflowY:'auto', background:'#00171f' }}>

      {/* Subject + Class selector bar */}
      <div style={{ background:'#001e2a', borderBottom:'1px solid #003050', padding:'0 32px', position:'sticky', top:0, zIndex:10 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap' }}>
          {/* Subject tabs */}
          <div style={{ display:'flex' }}>
            {notesData.map((s, i) => {
              const col = i===0?'#ecffb0':i===1?'#226ce0':'#34d399';
              return (
                <button key={s.subject}
                  onClick={() => { setActiveSubject(i); setActiveClass(0); }}
                  style={{ padding:'16px 24px', background:'none', border:'none', cursor:'pointer',
                    fontFamily:'DM Sans', fontSize:'0.9rem', fontWeight:600,
                    color: activeSubject===i ? col : 'rgba(250,255,216,0.38)',
                    borderBottom: activeSubject===i ? `2px solid ${col}` : '2px solid transparent',
          ...
          <div style={{ display:'flex', gap:'8px', padding:'12px 0' }}>
            {subject.classes.map((cls, i) => (
              <button key={cls.label} onClick={() => setActiveClass(i)}
                style={{ padding:'6px 18px', borderRadius:'20px',
                  border:`1px solid ${activeClass===i ? accentColor : 'rgba(250,255,216,0.15)'}`,
                  cursor:'pointer', fontFamily:'DM Sans', fontSize:'0.8rem', fontWeight:500,
                  background: activeClass===i ? accentColor+'18' : 'transparent',
                  color: activeClass===i ? accentColor : 'rgba(250,255,216,0.4)',
      ...
      {/* Page header */}
      <div style={{ padding:'28px 32px 16px', maxWidth:'860px', margin:'0 auto', width:'100%', boxSizing:'border-box' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'14px', marginBottom:'6px' }}>
          <span style={{ fontSize:'2rem' }}>{subject.icon}</span>
          <div>
            <div style={{ fontFamily:'Playfair Display', fontSize:'1.6rem', fontWeight:700, color:'#faffd8' }}>
              {subject.subject} Notes
            </div>
            <div style={{ fontSize:'0.82rem', color:'rgba(250,255,216,0.38)', marginTop:'2px' }}>
              {classData.label} · {classData.chapters.length} chapters · {registered ? 'Click to open in new tab ↗' : 'Free preview available — sign up for full access'}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Chapter list */}
      <div style={{ padding:'0 32px 32px', maxWidth:'860px', margin:'0 auto', width:'100%', boxSizing:'border-box' }}>
        <div style={{ display:'flex', flexDirection:'column', gap:'0' }}>
          {classData.chapters.map((ch, i) => (
            <button key={ch.name} onClick={() => handleChapterClick(ch)}
              style={{ display:'flex', alignItems:'center', gap:'20px',
                padding:'18px 20px',
                background:'transparent',
                border:'none',
                borderBottom:'1px solid rgba(34,108,224,0.12)',
                borderLeft:`3px solid ${i % 2 === 0 ? accentColor : 'transparent'}`,
                cursor:'pointer', textAlign:'left', width:'100%', boxSizing:'border-box',
                transition:'all 0.12s' }}
              onMouseEnter={e => { e.currentTarget.style.background='#001e2a'; e.currentTarget.style.borderLeftColor=accentColor; }}
              onMouseLeave={e => { e.currentTarget.style.background='transparent'; e.currentTarget.style.borderLeftColor = i%2===0 ? accentColor : 'transparent'; }}>

              {/* Number badge */}
              <span style={{ fontFamily:'JetBrains Mono', fontSize:'0.8rem', color:'rgba(250,255,216,0.25)',
                minWidth:'32px', fontWeight:500, flexShrink:0 }}>
                {String(i+1).padStart(2,'0')}
              </span>

              {/* Chapter name */}
              <span style={{ fontFamily:'DM Sans', fontSize:'0.98rem', color:'#faffd8', fontWeight:500, flex:1, textAlign:'left' }}>
                {ch.name}
              </span>

              {/* Action label */}
              <span style={{ fontSize:'0.75rem', color: registered ? accentColor : 'rgba(250,255,216,0.3)',
                fontFamily:'DM Sans', fontWeight:500, flexShrink:0 }}>
                {registered ? 'Open PDF ↗' : 'Preview →'}
              </span>
            </button>
          ))}
        </div>

        {/* Free signup CTA — only for guests */}
        {!registered && (
          <div style={{ marginTop:'32px', padding:'24px 28px',
            background:'linear-gradient(135deg, #001e2a 0%, #002535 100%)',
            border:'1px solid rgba(34,108,224,0.25)', borderRadius:'14px',
            display:'flex', alignItems:'center', justifyContent:'space-between',
            flexWrap:'wrap', gap:'16px' }}>
            <div>
              <div style={{ fontFamily:'Playfair Display', fontSize:'1.05rem', fontWeight:700, color:'#faffd8', marginBottom:'5px' }}>
                Get full access to all {classData.chapters.length} chapters
              </div>
              <div style={{ fontSize:'0.8rem', color:'rgba(250,255,216,0.42)', lineHeight:1.6 }}>
                Free forever · No payment · Instant access after signup
              </div>
            </div>
            <button onClick={onSignup}
              style={{ padding:'12px 28px', borderRadius:'10px', background:'#226ce0', border:'none',
                cursor:'pointer', fontFamily:'DM Sans', fontWeight:700, fontSize:'0.9rem', color:'#faffd8',
                whiteSpace:'nowrap', boxShadow:'0 4px 20px rgba(34,108,224,0.35)' }}>
              Create Free Account →
            </button>
          </div>
        )}
...
export default function App() {
  ...
  const handleRegistered = (name) => { setUserName(name); setRegistered(true); setShowRegPopup(false); setPage('app'); setMode('notes'); };
  ...
  const goToNotes      = () => { setPage('app'); setMode('notes'); };

  if (page === 'landing') {
    return (
      <>
        {showRegPopup && <RegistrationPopup onComplete={handleRegistered} />}
        <LandingPage onGetStarted={goToNotes} onTryCalculator={goToCalculator} />
      </>
    );
  }
  ...
  return (
    <div style={{display:'flex',height:'100vh',background:'#002535',fontFamily:'DM Sans',color:'#faffd8',overflow:'hidden',flexDirection:'column'}}>
      ...
      <div style={{height:'52px',minHeight:'52px',background:'#001e2a',borderBottom:'1px solid #003050',display:'flex',alignItems:'center',padding:'0 24px',gap:'0',justifyContent:'space-between'}}>
        ...
        <div style={{display:'flex',gap:'4px',background:'#002535',borderRadius:'10px',padding:'4px'}}>
          {[['calculator','🧮 Calculator'],['notes','📚 JEE Notes']].map(([m,label])=>(
            <button key={m} onClick={()=>{ setMode(m); }} style={{padding:'6px 20px',borderRadius:'7px',border:'none',cursor:'pointer',fontFamily:'DM Sans',fontSize:'0.85rem',fontWeight:500,transition:'all 0.15s',background:mode===m?'#226ce0':'transparent',color:mode===m?'#faffd8':'rgba(250,255,216,0.45)'}}>{label}</button>
          ))}
        </div>
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden'}}>
        {mode === 'notes' ? <NotesSection registered={registered} onSignup={openSignup} /> : (
          <>
